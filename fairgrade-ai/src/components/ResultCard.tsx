...
import { RotateCcw, AlertTriangle, Clock, CheckCircle, Edit3, ShieldCheck, ZoomIn, Eye, TrendingDown, TrendingUp, Info } from 'lucide-react';
...
const ResultCard: React.FC<ResultCardProps> = ({ result, studentId, onRetry, onVerify, truncateName }) => {
  ...
  return (
    <div className="result-card" style={{ animation: "fadeInUp 0.5s ease-out", marginBottom: '1.25rem' }}>
      ...
      {!isQuotaIssue && report.ocrConfidence !== undefined && report.ocrConfidence < 0.9 && (
        <div style={{
          marginTop: '0.75rem', padding: '0.65rem 1rem',
          borderRadius: '10px', display: 'flex', gap: '0.6rem', alignItems: 'flex-start',
          background: report.flagManualReview
            ? 'rgba(239,68,68,0.06)'
            : 'rgba(245,158,11,0.07)',
          border: `1px solid ${report.flagManualReview
            ? 'rgba(239,68,68,0.22)'
            : 'rgba(245,158,11,0.22)'}`,
          fontSize: '0.82rem',
        }}>
          <Eye size={16} style={{ color: report.flagManualReview ? '#ef4444' : '#d97706', flexShrink: 0, marginTop: '2px' }} />
          <div>
            <strong style={{ color: report.flagManualReview ? '#ef4444' : '#d97706' }}>
              {report.flagManualReview ? '⚠ Low OCR Confidence — Manual Review Recommended' : 'OCR Quality Notice'}
            </strong>
            <p style={{ margin: '0.25rem 0 0', color: 'var(--text-muted)' }}>
              Extraction confidence: <strong>{(report.ocrConfidence * 100).toFixed(0)}%</strong>.
              {report.flagManualReview
                ? ' The text may be too unclear for reliable bias analysis. Please review the original paper.'
                : ' Minor extraction uncertainty detected — bias score has been adjusted automatically.'}
            </p>
          </div>
        </div>
      )}
      ...
      {!isQuotaIssue && report.directionalConsistency && report.directionalConsistency !== 'mixed' && (
        <div style={{
          marginTop: '0.75rem', padding: '0.65rem 1rem',
          borderRadius: '10px', display: 'flex', gap: '0.6rem', alignItems: 'center',
          background: 'rgba(124,58,237,0.05)',
          border: '1px solid rgba(124,58,237,0.18)',
          fontSize: '0.82rem',
        }}>
          {report.directionalConsistency === 'consistent_undergrading'
            ? <TrendingDown size={16} color="#d97706" />
            : <TrendingUp size={16} color="#dc2626" />}
          <span>
            <strong style={{ color: 'var(--primary)' }}>Systematic Pattern Detected: </strong>
            <span style={{ color: 'var(--text-muted)' }}>
              {report.directionalConsistency === 'consistent_undergrading'
                ? 'Teacher consistently scores below AI across this session — possible systematic undergrading.'
                : 'Teacher consistently scores above AI across this session — possible systematic overgrading.'}
            </span>
          </span>
        </div>
      )}
      ...
      {/* ── Per-Question Bias Explainability ── */}
      ...
      {/* ── Methodology Note — AI is not ground truth ── */}
      {!isQuotaIssue && report.biasMethodologyNote && (
        <div style={{
          marginTop: '0.75rem', padding: '0.55rem 0.9rem',
          borderRadius: '8px', display: 'flex', gap: '0.5rem', alignItems: 'flex-start',
          background: 'rgba(100,116,139,0.05)', border: '1px solid rgba(100,116,139,0.15)',
          fontSize: '0.72rem', color: 'var(--text-muted)', lineHeight: 1.5,
        }}>
          <Info size={12} style={{ flexShrink: 0, marginTop: '2px', opacity: 0.6 }} />
          <span>{report.biasMethodologyNote}</span>
        </div>
      )}
    </div>
  );
