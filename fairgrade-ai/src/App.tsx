...
function App() {
  ...
  const [backendStatus, setBackendStatus] = useState<'connecting' | 'online' | 'offline' | 'waking'>('connecting');

  useEffect(() => {
    // Render free-tier cold-starts in ~30s. We retry 4 times with backoff
    // before declaring offline, and show 'Waking up...' during that window.
    let cancelled = false;
    const MAX_ATTEMPTS = 4;
    const TIMEOUTS = [8000, 10000, 12000, 15000]; // progressive timeout per attempt
    ...
    const ping = async (attempt: number): Promise<void> => {
      ...
      try {
        ...
        const tid = setTimeout(() => controller.abort(), TIMEOUTS[attempt - 1] ?? 15000);
        ...
        if (!cancelled) {
          if (attempt < MAX_ATTEMPTS) {
            setTimeout(() => ping(attempt + 1), 5000); // wait 5s between retries
          } else {
    ...
    ping(1);
    // Keep-alive re-check every 10 minutes to detect recovery
    const keepAlive = setInterval(() => ping(1), 10 * 60 * 1000);
  ...
  const retryFile = async (fileName: string): Promise<void> => {
    ...
    try {
      ...
      // Pass Firebase UID so backend can fetch cross-session history for DCS
      if (user?.uid) formData.append("teacher_id", user.uid);
  ...
  const runAgents = async (): Promise<void> => {
    ...
    try {
      ...
      setCurrentStep(-1);
      // Show slide-in completion badge for 3s
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <span
            ...
            title={
              backendStatus === 'offline' ? 'Backend is offline — check your connection or try again shortly' :
              backendStatus === 'waking'  ? 'Render free-tier is cold-starting (~30s) — please wait' : ''
            }
            style={{
              ...
              background:
                backendStatus === 'online'     ? 'rgba(5,150,105,0.1)'   :
                backendStatus === 'offline'    ? 'rgba(239,68,68,0.1)'   :
                backendStatus === 'waking'     ? 'rgba(245,158,11,0.1)'  :
                                                 'rgba(124,58,237,0.1)',
              color:
                backendStatus === 'online'     ? '#059669' :
                backendStatus === 'offline'    ? '#ef4444' :
                backendStatus === 'waking'     ? '#d97706' :
                                                 'var(--primary)',
              border: `1px solid ${
                backendStatus === 'online'     ? 'rgba(5,150,105,0.25)'  :
                backendStatus === 'offline'    ? 'rgba(239,68,68,0.25)'  :
                backendStatus === 'waking'     ? 'rgba(245,158,11,0.25)' :
                                                 'rgba(124,58,237,0.2)'
              }`,
            }}
          >
            ...
            {backendStatus === 'connecting' ? 'Connecting…'
              : backendStatus === 'waking'  ? 'Waking up… (~30s)'
          ...
          <div style={demoMode ? { gridColumn: '1 / -1' } : {}}>
            ...
            {!demoMode && (isProcessing || currentStep > -1) ? (
              <>
                ...
                {/* AI Thoughts stream — fills the waiting area, no skeleton needed */}
                <AIThoughtsStream />
              </>
            ...
            {displayResults.length > 0 && (
              <div className="results-container" id="printable-results">
                {/* Slide-in completion banner */}
                <div style={{
                  overflow: 'hidden',
                  maxHeight: showComplete ? '60px' : '0px',
                  opacity: showComplete ? 1 : 0,
                  transform: showComplete ? 'translateX(0)' : 'translateX(-40px)',
                  transition: 'max-height 0.4s ease, opacity 0.4s ease, transform 0.4s cubic-bezier(0.34,1.56,0.64,1)',
                  marginBottom: showComplete ? '1rem' : '0',
                }}>
                  <div style={{
                    ...
                    Pipeline complete — {results.filter(r => !r.error).length}/{results.length} evaluated
                  </div>
                </div>
